...
static const char const BASE_32_ALPHABET[] = {'1', '3', '4', '5', '6', '7', '8', '9', 'A', 'B', 'C',
...
/** encodes in_length bytes from in into base-2, writes the converted bytes to out, stopping when
 * it converts out_length bytes.  */
unsigned int encode_base_2(const void *in,
                           const unsigned int in_length,
                           char *out,
                           const unsigned int out_length,
                           const bool enable_debug) {
    return encode_base_x(BASE_2_ALPHABET,
 ...
 * it converts out_length bytes.  */
unsigned int encode_base_10(const void *in,
                            const unsigned int in_length,
                            char *out,
                            const unsigned int out_length,
                            const bool enable_debug) {
...
/** encodes in_length bytes from in into base-32, writes the converted bytes to out, stopping when
 * it converts out_length bytes.  */
unsigned int encode_base_32(const void *in,
                            const unsigned int in_length,
                            char *out,
                            const unsigned int out_length,
                            const bool enable_debug) {
    return encode_base_x(BASE_32_ALPHABET,
                         sizeof(BASE_32_ALPHABET),
                         in,
                         in_length,
                         out,
                         out_length,
 ...
 */
static unsigned int divide_and_remainder(const unsigned char *divided,
                                         const unsigned int divided_len,
                                         unsigned char *dividend,
                                         const unsigned int dividend_len,
    ...
    while (division_index < divided_len) {
        ...
        // I.E: if you are dividing 31 by 4, and the divided part is 3, multiply 3 by 10 and add
        // the 1.
 ...
 * 2) Do long division, dividing the input by the divisor, saving the dividend_length,
...
static unsigned int encode_base_x(const char *alphabet,
    ...
    // remainders_out, the pointer to the first remainder that will be saved (all before this will
    // be zero).
    ...
    while (!empty_divided) {
        ...
        const unsigned int remainder = divide_and_remainder(divided,
                                                            working_len,
                                                            dividend,
                                                            working_len,
                                                            divisor,
