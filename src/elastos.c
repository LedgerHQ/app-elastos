...
void public_key_hash160(unsigned char *in,
...
static void to_address(char *dest,
...
void display_public_key(const unsigned char *public_key) {
    ...
    public_key_hash160(verification_script,
...
static void to_base10_100m(char *dest, const unsigned char *value, const unsigned int dest_len) {
    ...
    unsigned int buffer_len =
        encode_base_10(reverse_value, VALUE_LEN, base10_buffer, MAX_TX_TEXT_WIDTH, false);
    ...
    if (buffer_len < DECIMAL_PLACE_OFFSET) {
        ...
        os_memmove(dest + dec_place_ix + 1,
                   base10_buffer + dec_place_ix,
                   buffer_len - dec_place_ix);
    }
}

/** array of capital letter hex values */
static const char const HEX_CAP[] = {
    '0',
    '1',
    '2',
    '3',
...
void display_tx_desc() {
    ...
    for (int output_ix = 0; output_ix < num_outputs; output_ix++) {
        ...
        if (scr_ix < MAX_TX_TEXT_SCREENS) {
            ...
            for (int zero_ix = 0; (zero_ix < MAX_TX_TEXT_WIDTH - 2) && (found_nonzero == 0);
                 zero_ix++) {
